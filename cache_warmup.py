...
date_from = date_to = CantonService.get_default_date().strftime(df)
...
for canton, canton_data in cantonsservice_urls.items():
    ...
    if canton_data['url'] != '':
        ...
        municipalities, status = CantonService.get_municipalities(canton)
        ...
        if municipalities is not None:
            for municipality in municipalities:
                ...
                incidence_data, status = CantonService.get_incidences(
                ...
                while retry_count < 3 and incidence_data == []:
                    ...
                    incidence_data, status = CantonService.get_incidences(municipality['canton'], (CantonService.get_default_date() - delta_days).strftime(
                        df), (CantonService.get_default_date() - delta_days).strftime(df), municipality['bfsNr'])
